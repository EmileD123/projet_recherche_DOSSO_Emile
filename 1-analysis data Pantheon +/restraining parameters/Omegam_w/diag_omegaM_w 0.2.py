    ...
    for j in range(len(OmegaMatter)):
        for k in range(len(W_eq_of_state)):
            ...
            for i in range(1701):
                ...
                def f(x):
                    return (1/(73.5*((OmegaMatter[j][j]*((1+x)**3)+0.691*((1+x)**(3*(1+W_eq_of_state[k][k]))))**(1/2))))*(3*(10**5))*(10**6)  #🔴Omegam divided by 100 here ! ; calculation of the luminous distance with cosmological parameters (OmegaLambda corresponding to the flat ΛCDM in Brout et al. 2022 = Analysis on cosmological constraints)
