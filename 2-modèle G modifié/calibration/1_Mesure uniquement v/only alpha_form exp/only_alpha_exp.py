    ...
    for j in range(len(alpha)):
        ...
        for i in range(1701):
            ...
            if CEPH_DIST[i] == -9.0 : #on vérifie si la mesure est relié à la mesure d'une distance avec une Cepheid (CEPH_DIST[i] == -9.0 signifie que ce n'est pas le cas)
                    mu = (mu_shoes + (5/2)*np.log10((1+alpha[j]*np.exp((zHD[i]/(1+zHD[i]))))/(1+alpha[j])))-mu_theory
                    DeltaD[i]=mu
            else :
                mu = (mu_shoes + (5/2)*np.log10((1+alpha[j]*np.exp((zHD[i]/(1+zHD[i]))))/(1+alpha[j])))-mu_cepheid
                DeltaD[i]=mu
