    ...
    for j in range(len(gamma)):
        for k in range(len(beta)):
            ...
            for i in range(1701):
                ...
                def f(x):
                    return (1/(((1+0.18*np.exp(x/((1+x)*beta[k][k])))**(gamma[j][j]/2))*(73.6)*((0.334*((1+x)**3)+0.666)**(1/2))))*(3*(10**5))*(10**6) #🔴calcul de la distance lumineuse avec les paramètres cosmologiques FlatLambdaCDM dans Brout et al. 2022 = Analysis on cosmological constraints
...
delta = 1
gamma_range = np.arange(-10, 11, delta) 
