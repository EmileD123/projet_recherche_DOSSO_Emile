...
def likelihood_func(alpha,beta,gamma,mat_cov,zHD,CEPH_DIST,MU_SHOES) :
    likelihood=[]
        ...
        for k in range(len(beta)):
            for l in range(len(gamma)):
                ...
                for i in range(1701):
                    ...
                    def f(x):
                        return (1/(((1+alpha[j][j][j]*np.exp(x/((1+x)*beta[k][k][k])))**(1/2))*(73.6)*((0.334*((1+x)**3)+0.666)**(1/2))))*(3*(10**5))*(10**6) #🔴calcul de la distance lumineuse avec les paramètres cosmologiques FlatLambdaCDM dans Brout et al. 2022 = Analysis on cosmological constraints
                    ...
                    if CEPH_DIST[i] == -9.0 : #on vérifie si la mesure est relié à la mesure d'une distance avec une Cepheid (CEPH_DIST[i] == -9.0 signifie que ce n'est pas le cas)
                        mu = (mu_shoes + (5/2)*gamma[l][l][l]*np.log10(1+alpha[j][j][j]*np.exp(zHD[i]/((1+zHD[i])*beta[k][k][k])))**(1/2)) - mu_theory
                        DeltaD[i]=mu
                    else :
                        mu = (mu_shoes + (5/2)*gamma[l][l][l]*np.log10(1+alpha[j][j][j]*np.exp(zHD[i]/((1+zHD[i])*beta[k][k][k])))**(1/2)) - mu_cepheid 
                        DeltaD[i]=mu
                DeltaD_transpose = np.transpose(DeltaD)
                    #on calcule la likelihood 
                A = np.dot(DeltaD_transpose,np.linalg.inv(mat_cov))
                likelihood[j][k].append(np.dot(A,DeltaD)) 
    return likelihood
...
delta = 0.1
alpha_range = np.arange(-0.5, 0.6, delta) 
beta_range = np.arange(-0.5, 0.6, delta)
gamma_range = np.arange(-0.5,0.6,delta)                 
alpha_range, beta_range , gamma_range = np.meshgrid(alpha_range,beta_range,gamma_range)
alpha_range = alpha_range.astype(float);  beta_range = beta_range.astype(float) ; gamma_range = gamma_range.astype(float)
Z = likelihood_func(alpha_range,beta_range,gamma_range,matcov_SN_Cepheid,zHD,CEPH_DIST,MU_SHOES)
Z = np.array(Z) ; Z = Z.astype(float)
#Z[0][10] = 0 on supprme un outlier (valeur à 10**8)
#Z[0][0] = 10000
print(Z) ; print(alpha_range, beta_range,gamma_range)
...
for i in range(len(Z)) :
    for j in range(len(Z[i])) :
        for k in range(len(Z[i][j])) :
            if min >= Z[i][j][k] and Z[i][j][k] != 0 and Z[i][j][k] != nan:
                min = Z[i][j][k] ; # ??? -> arg_min_alpha = alpha_range[0][i][0] ; arg_min_beta = beta_range[j][0][0]; arg_min_gamma = beta_range[0][0][k] 
