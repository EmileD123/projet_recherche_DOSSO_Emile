    ...
    for j in range(len(alpha)):
        ...
        for i in range(1701):
            ...
            if CEPH_DIST[i] == -9.0 : #on vérifie si la mesure est relié à la mesure d'une distance avec une Cepheid (CEPH_DIST[i] == -9.0 signifie que ce n'est pas le cas)
                mu = (mu_shoes + (5/2)*np.log10((1+alpha[j]*(1/(1+zHD[i])))/(1+alpha[j])))-mu_theory #alpha = 0.18
                DeltaD[i]=mu
            else :
                mu = (mu_shoes + (5/2)*np.log10((1+alpha[j]*(1/(1+zHD[i])))/(1+alpha[j])))-mu_cepheid
                DeltaD[i]=mu
...
delta = 0.01
alpha = np.arange(-0.5, 0.51, delta) 
