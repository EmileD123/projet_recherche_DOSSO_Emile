...
MU_SHOES_alpha_fixé_gamma_H0 = []



for i in range(1701):
        #alpha_fixé_gamma_H0 : 
        alpha_brout_f_lcdm = 0.192 ; gamma_optim = -0.6 + 0.4 # alpha_riess = 0.18 ; gamma_optim = -0.5
        mu_alpha_fixé_gamma_H0 = MU_SHOES[i] + (5/2)*gamma_optim*np.log10((1+alpha_brout_f_lcdm*(1/(1+zHD[i])))/(1+alpha_brout_f_lcdm))
        MU_SHOES_alpha_fixé_gamma_H0.append(mu_alpha_fixé_gamma_H0)
        """
...
MU_alpha_fixé_gamma_H0 = []  
alpha_brout_f_lcdm = 0.192 ; H0_optim = 67.1 #alpha_riess = 0.18 ; H0_optim = 67.5 ;
for z in zHD:
    def f(x):
        return (1/(((1+alpha_brout_f_lcdm*(1/(1+x)))**(1/2))*H0_optim*(((0.334)*((1+x)**3)+(0.666))**(1/2))))*(3*(10**5))*(10**6)   
    result = spi.quad(f,0,z)
    result_final = 5*np.log10(((1+z)*result[0])/10) 
    MU_alpha_fixé_gamma_H0.append(result_final)  
...
plt.scatter(zHD,MU_SHOES_alpha_fixé_gamma_H0, c='red', marker='.', label='alpha_fixé_gamma_H0 : alpha = 0.192 , gamma = -0.6 + 0.4  , H0 = 67.1')
plt.plot(zHD,MU_theory_flatLCDM,c='green',label='Flat LCDM')
plt.plot(zHD,MU_alpha_fixé_gamma_H0 ,c='r',label='alpha_fixé_gamma_H0 : alpha = 0.192 , gamma = -0.6 +0.4 , H0 = 67.1')
plt.legend()

residuals_sans_modif = []
for i in range(len(zHD)):
    if CEPH_DIST[i] == -9.0 : 
        mu = MU_SHOES[i] - MU_theory_flatLCDM[i] 
    else :
        mu = MU_SHOES[i] - CEPH_DIST[i]
    residuals_sans_modif.append(mu)
...
for i in range(len(zHD)):
    if CEPH_DIST[i] == -9.0 : 
        mu = MU_SHOES_alpha_fixé_gamma_H0[i] - MU_alpha_fixé_gamma_H0[i] 
    else :
        mu = MU_SHOES_alpha_fixé_gamma_H0[i] - CEPH_DIST[i]
    residuals_avec_modif.append(mu)
...
plt.scatter(zHD,residuals_avec_modif,c='red',label='alpha_fixé_gamma_H0 : alpha = 0.192 , gamma = -0.6 + 0.4, H0 = 67.1')
plt.legend()
plt.show()
