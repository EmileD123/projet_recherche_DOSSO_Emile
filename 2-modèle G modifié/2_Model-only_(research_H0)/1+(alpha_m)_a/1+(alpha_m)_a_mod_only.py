...
def likelihood_func(m,H0,mat_cov,zHD,CEPH_DIST,MU_SHOES) :
    likelihood=[]
    ...
    for j in range(len(H0)):
        for k in range(len(m)):
            ...
            for i in range(1701):
                ...
                def f(x):
                    return (1/(((1+((73.3/67.4)**(2/m[k][k])-1)*(1/(1+x)))**(m[k][k]/2))*H0[j][j]*(((0.334)*((1+x)**3)+(0.666))**(1/2))))*(3*(10**5))*(10**6) #alpha_m = ((H0riess/H0planck)**(2/m)-1) = ((73.3/67.4)**(2/m)-1)
                    #↑we use the value of OmegaM = 0.334 optimized for the flat ΛCDM 
...
delta = 1
H0 = np.arange(65,75.5,delta/2) 
m = np.arange(-5,5.5,delta/2)
H0 , m = np.meshgrid(H0, m,indexing='ij')
H0 = H0.astype(float); m = m.astype(float)
Chi2 = likelihood_func(m,H0,matcov_SN_Cepheid,zHD,CEPH_DIST,MU_SHOES)
Chi2 = np.array(Chi2) ; Chi2 = Chi2.astype(float)


min = Chi2[0][1]
CI_1σ = [] ; CI_2σ = [] ; CI_1σ_H0 = []; CI_1σ_m = []; CI_1σ_m_chi2 = [] 

for i in range(len(Chi2)) :
    for j in range(len(Chi2[i])) :
        if min >= Chi2[i][j] and Chi2[i][j] != 0 and Chi2[i][j] != nan:
            min = Chi2[i][j] ; arg_min_H0 = H0[i][j] ; arg_min_m =m[i][j]
print("H0= ", arg_min_H0, "; m= ", arg_min_m, "; min =", min)


for i in range(len(Chi2)) :
    for j in range(len(Chi2[i])) :
        #if min <= Chi2[i][j] and Chi2[i][j]<=min+6.17 and Chi2[i][j] != nan:
        #CI_2σ.append([Chi2[i][j]]);CI_2σ.append(H0[i][j]);CI_2σ.append(m[i][j])
            
        if min <= Chi2[i][j] and Chi2[i][j]<=min+2.3 and Chi2[i][j] != nan:
           # CI_1σ.append([Chi2[i][j]]);CI_1σ.append(H0[i][j]);CI_1σ.append(m[i][j])
            CI_1σ_H0.append(H0[i][j])
            CI_1σ_m.append(m[i][j])
            CI_1σ_m_chi2.append(Chi2[i][j])

merge_sort(CI_1σ_H0);merge_sort(CI_1σ_m)
