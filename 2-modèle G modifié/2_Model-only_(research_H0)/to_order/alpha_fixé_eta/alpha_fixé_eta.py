...
def likelihood_func(eta,mat_cov,zHD,CEPH_DIST,MU_SHOES) :
    likelihood=[]
    for k in range(len(eta)):
        ...
        for i in range(1701):
            ...
            def f(x):
                return (1/(((1+(0.18)*((1/(1+x))**eta[k]))**(1/2))*(73.6)*((0.334*((1+x)**3)+0.666)**(1/2))))*(3*(10**5))*(10**6) #🔴calcul de la distance lumineuse avec les paramètres cosmologiques FlatLambdaCDM dans Brout et al. 2022 = Analysis on cosmological constraints
...
#maintenant on va tenter de tracer le diagramme (eta_range,eta) en faisant varier ces paramètres et trouver le minimum de la likelihood
delta = 100000
