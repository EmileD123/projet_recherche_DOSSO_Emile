...
def likelihood_func(H0,mat_cov,zHD,CEPH_DIST,MU_SHOES) :
    ...
    for j in range(len(H0)):
        ...
        for i in range(len(mat_cov)):
            ...
            def f(x):
                return (1/(((1+0.088*(1/(1+x)))**(2/2))*H0[j]*(((0.334)*((1+x)**3)+(0.666))**(1/2))))*(3*(10**5))*(10**6)  
                #↑we use the value of OmegaM = 0.334 optimized for the flat ΛCDM ; alpha_2 = 0.088
...
print("H0 - 1σ = ",CI_1σ[0], "; H0 + 1σ = ",CI_1σ[-1])
#print("CI_2σ=", CI_2σ)
