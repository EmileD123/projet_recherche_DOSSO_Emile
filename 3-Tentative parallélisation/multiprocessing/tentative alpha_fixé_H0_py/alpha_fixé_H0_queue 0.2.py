...
def calculate(args):
    result = likelihood_func(*args)
    return result


#on définit la fonction qui calcule la likelihood : ici prend le rôle de "worker function"
...
def parallel_likelihood_func():
    
    NUMBER_OF_PROCESSES = 2
    delta = 1
    H0 = np.arange(65, 75.1, delta) 
    H0 = H0.astype(float)
    nb_elements_per_process = len(H0) // NUMBER_OF_PROCESSES

    TASK = [(H0[i * nb_elements_per_process : i * nb_elements_per_process + nb_elements_per_process if i < NUMBER_OF_PROCESSES - 1 else len(H0)],matcov_SN_Cepheid,zHD,CEPH_DIST,MU_SHOES) for i in range(NUMBER_OF_PROCESSES)]
    ...
    while not done_queue.empty():
        likelihood_grid.extend(done_queue.get())
...
if __name__ == "__main__":
    ...
    # on redéfinit H0 car on a plus accès à l'ancienne version étant donné qu'on est dans le main
    delta = 0.1
    H0 = np.arange(65, 75.1, delta) 
    H0 = H0.astype(float)
