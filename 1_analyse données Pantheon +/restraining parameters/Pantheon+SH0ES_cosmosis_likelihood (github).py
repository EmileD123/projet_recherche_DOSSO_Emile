...
class PantheonLikelihood(GaussianLikelihood):
    ...
    def extract_theory_points(self, block):
        ...
        theory_ynew[np.array(self.is_calibrator,dtype='bool')] = self.cepheid_distance[np.array(self.is_calibrator,dtype='bool')]

        # Actually do the interpolation at the data redshifts
        zcmb = self.zCMB[~np.array(self.is_calibrator,dtype='bool')]
        zhel = self.zHEL[~np.array(self.is_calibrator,dtype='bool')]
        theory_ynew[~np.array(self.is_calibrator,dtype='bool')] = 5.0*np.log10((1.0+zcmb)*(1.0+zhel)*np.atleast_1d(f(zcmb)))+25.
