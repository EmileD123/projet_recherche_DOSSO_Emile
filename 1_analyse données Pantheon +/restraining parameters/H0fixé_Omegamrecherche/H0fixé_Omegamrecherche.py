...
def likelihood_func(OmegaMatter,mat_cov,zHD,CEPH_DIST,MU_SHOES) :
    ...
    for j in range(len(OmegaMatter)):
        ...
        for i in range(1701):
            ...
            def f(x):
                return (1/(73.6*(((OmegaMatter[j]/100)*((1+x)**3)+(1-(OmegaMatter[j]/100)))**(1/2))))*(3*(10**5))*(10**6) #🔴Omegam divisé par 100 ici ! ; calcul de la distance lumineuse avec les paramètres cosmologiques (OmegaLambda correspondant au flat ΛCDM dans Brout et al. 2022 = Analysis on cosmological constraints)
...
for i in range(len(Chi2)) :
        if min <= Chi2[i] and Chi2[i]<=min+3.841 and Chi2[i] != nan:
            CL_95.append([Chi2[i]]);CL_95.append(OmegaM[i])
            

        if min <= Chi2[i] and Chi2[i]<=min+1 and Chi2[i] != nan:
            CL_68.append(Chi2[i]);CL_68.append(OmegaM[i])
