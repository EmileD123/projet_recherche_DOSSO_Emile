    ...
    for j in range(len(gamma)):
        for k in range(len(H0)):                
            ...
            for i in range(1701):
                ...
                def f(x):
                    return (1/(((1+alpha*np.exp(-(1-(1/(1+x)))/(1/(1+x))))**(1/2))*H0[k][k]*(((0.333)*((1+x)**3)+(0.667))**(1/2))))*(3*(10**5))*(10**6)
                ...
                if CEPH_DIST[i] == -9.0 : #we check whether the measurement is related to the measurement of a distance with a Cepheid (CEPH_DIST[i] == -9.0 means that this is not the case)
                    mu = (mu_shoes + (5/2)*gamma[j][j]*np.log10((1+alpha*np.exp(-(1-(1/(1+zHD[i])))/(1/(1+zHD[i]))))/(1+alpha)))-mu_theory #alpha = 0.18 ; alpha_brout = 0.192
                    DeltaD[i]=mu 
                else :
                    mu = (mu_shoes + (5/2)*gamma[j][j]*np.log10((1+alpha*np.exp(-(1-(1/(1+zHD[i])))/(1/(1+zHD[i]))))/(1+alpha)))-mu_cepheid #alpha = 0.18 ; alpha_brout = 0.192
                    DeltaD[i]=mu
...
delta = 0.01
gamma = np.arange(-1.5,0.51,delta) 
H0 = np.arange(66.5, 68.51, delta) ; H0 = H0[:-1]               
...
im = ax.imshow(Chi2, interpolation ='bilinear',
               origin ='lower',
               cmap ="bone",extent=(66.5,68.5,-1.5,0.5))
  
levels = [min+2.3,min+6.7]
CS = ax.contour(Chi2, levels, 
                origin ='lower',
                cmap ='Greens',
                linewidths = 2,extent=(66.5,68.5,-1.5,0.5))
...
plt.savefig('confidence_contour_H0_gamma_[66.5;68.5]_[-1.5;0.5].jpg')
